...
class Payment
{
    ...
    public function __construct(array $api_data)
    {
        try {
            ...
            if (empty($this->api_data['extra'])) {
                throw new Exception('You must set your API Signature');
            }
    ...
    public function process()
    {
        try {
            ...
            $gateway->initialize([
                ...
                'password' => $this->api_data['password'],
                'vendor' => $this->api_data['extra'],
                'partner' => 'PayPal',
            ...
            if ($response->isSuccessful()) {
                $reference = json_decode($response->getTransactionReference(), 1);
                $this->return['answer'] = 'SUCCESS';
                $this->return['trans_id'] = $reference['transId'];
